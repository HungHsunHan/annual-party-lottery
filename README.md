# 公司尾牙抽獎系統

一個可打包成獨立執行檔的雙視窗抽獎系統，專為公司尾牙活動設計。

## 功能特色

- 🎰 **雙視窗架構**：後台控制 + 前台投影畫面
- 📊 **Excel 匯入匯出**：人員名單、獎項名單、中獎名單
- 🎨 **自訂素材**：Logo、背景、音效可自行更換
- 🧩 **前台畫面設定**：標題文案、顯示區塊、倒數/揭曉配置可即時調整
- 🧲 **拖拉排序**：獎項可拖拉調整抽獎順序
- 💾 **自動備份**：每次確認中獎自動存檔
- 📸 **快照功能**：支援「時光倒流」還原狀態
- 🎯 **靈活抽獎**：逐一抽 / 一次全抽（可在開發設定啟用自訂數量）
- ✨ **動態特效**：抽獎動畫、紙花慶祝效果
- 🧯 **中斷續抽**：啟動時偵測未完成活動，可選擇續接
- 🔁 **名單插入合併**：支援把額外名單插入現有清單並去重
- 🎛️ **排除已中獎**：全域預設 + 單獎項可切換是否排除

## 快速開始

### 開發模式

```bash
# 安裝依賴
npm install

# 啟動開發伺服器（會同時開啟兩個視窗）
npm run dev
```

### 打包執行檔

```bash
# 打包成執行檔
npm run electron:build
```

打包完成後，執行檔會在 `release` 資料夾內。

## Excel 格式說明

> [!IMPORTANT]
> 為確保匯入成功，請務必遵守以下格式與命名規範。

### 支援的檔案格式

| 格式 | 副檔名 | 說明 |
|------|--------|------|
| Excel 2007+ | `.xlsx` | ✅ **建議使用** |
| Excel 97-2003 | `.xls` | ✅ 支援匯入 |

> [!TIP]
> 檔名可以自訂，例如 `2024尾牙人員.xlsx`、`獎項清單.xlsx` 等皆可匯入。

---

### 人員名單

#### 必要欄位（第一列必須是標題列）

| 欄位名稱 | 替代名稱（英文） | 必填 | 說明 |
|---------|-----------------|------|------|
| 部門 | department | ❌ 選填 | 員工部門名稱 |
| 職稱 | title | ❌ 選填 | 員工職稱 |
| 姓名 | name | ✅ **必填** | 員工姓名（不可為空） |

#### 進階欄位（可選，用於保留狀態或對應）

| 欄位名稱 | 替代名稱（英文） | 說明 |
|---------|-----------------|------|
| 參與者ID | participantId 或 id | 唯一識別值，未提供會自動生成 |
| 已中獎 | hasWon | 支援 `是/否`、`true/false`、`1/0` |

#### 範例資料

| 部門 | 職稱 | 姓名 |
|------|------|------|
| 研發部 | 工程師 | 王小明 |
| 行銷部 | 經理 | 李小華 |
| 財務部 | 會計 | 張小美 |

> [!NOTE]
> - 系統會讀取 **第一個工作表 (Sheet)**
> - 標題列可使用中文或英文，但必須**完全相符**（區分大小寫）
> - 姓名欄位為空的資料列會被自動忽略

---

### 獎項名單

#### 必要欄位（第一列必須是標題列）

| 欄位名稱 | 替代名稱（英文） | 必填 | 說明 |
|---------|-----------------|------|------|
| 獎項名稱 | prize 或 name | ✅ **必填** | 獎品名稱 |
| 數量 | quantity | ❌ 選填 | 獎項數量（預設為 1） |
| 圖示 | icon 或 iconPath | ❌ 選填 | 獎品圖片路徑（相對於執行檔同層資料夾） |

#### 進階欄位（可選，用於保留狀態或排序）

| 欄位名稱 | 替代名稱（英文） | 說明 |
|---------|-----------------|------|
| 獎項ID | prizeId 或 id | 唯一識別值，未提供會自動生成 |
| 總數量 | quantity | 與「數量」同義 |
| 已抽中數量 | drawnCount | 已抽中數量 |
| 排序順位 | order | 抽獎順序 |
| 排除已中獎者 | excludeWinners | `是/否`，單獎項排除設定 |
| 狀態 | status | `pending / in-progress / incomplete / completed` |

#### 範例資料

| 獎項名稱 | 數量 | 圖示 |
|---------|------|------|
| 現金 10000 元 | 1 |  |
| iPhone 15 | 2 | images/iphone.png |
| 家電禮券 | 5 | images/voucher.jpg |

> [!NOTE]
> - 系統會依照 Excel 中的順序匯入獎項
> - 若「數量」欄位為空或無此欄位，預設每獎項數量為 1
> - 「圖示」欄位可省略或留空，留空則前台不顯示圖片
> - 圖片路徑為相對路徑，基準為**執行檔同層資料夾**（portable 版可放在同層的 `images/`）
> - 開發模式（`npm run dev`）下基準為**專案根目錄**
> - 獎項名稱為空的資料列會被自動忽略

---

### 常見匯入錯誤

| 錯誤情況 | 原因 | 解決方式 |
|---------|------|---------|
| 匯入後資料為空 | 標題列名稱錯誤 | 確認標題列使用正確名稱（如「姓名」而非「名字」） |
| 部分欄位遺失 | 欄位名稱拼寫錯誤 | 檢查是否有多餘空白或錯字 |
| 無法讀取檔案 | 檔案格式不支援 | 使用 `.xlsx` 或 `.xls` 格式 |
| 資料順序混亂 | 非預期的工作表 | 確認資料在**第一個工作表**中 |
| 中獎匯入失敗 | 無法對應獎項或得獎者 | 請先匯入人員名單與獎項名單 |

---

### 中獎名單

> [!NOTE]
> 匯入中獎名單前，需先匯入人員名單與獎項名單。
> 匯入後系統會依中獎資料更新人員中獎狀態與獎項進度。

#### 主要欄位（第一列必須是標題列）

| 欄位名稱 | 替代名稱（英文） | 說明 |
|---------|-----------------|------|
| 中獎ID | winnerId 或 id | 唯一識別值 |
| 獎項ID | prizeId | 可用於精準對應獎項 |
| 獎項名稱 | prize 或 name | 可用於名稱對應 |
| 得獎者ID | participantId | 可用於精準對應人員 |
| 得獎者姓名 | name | 若無 ID，會以姓名/部門/職稱比對 |
| 部門 | department | 搭配姓名增加辨識度 |
| 職稱 | title | 搭配姓名增加辨識度 |
| 中獎時間 | drawnAt | 可為日期或文字 |
| 已確認 | confirmed | `是/否`、`true/false`、`1/0` |

## 操作說明

### 後台控制面板

1. **儀表板**：查看統計資訊、選擇獎項、開始抽獎
2. **獎項管理**：匯入/新增獎項、拖拉調整順序、調整數量
3. **人員名單**：匯入/新增人員、搜尋篩選
4. **中獎名單**：查看/匯入/匯出中獎紀錄、排序、刪除
5. **設定**：前台畫面設定、素材上傳、存檔/還原、管理快照

### 抽獎流程

1. 匯入人員名單和獎項
2. 選擇要抽的獎項
3. 點擊「開始抽獎」
4. 抽中後可選擇「確認領取」或「放棄重抽」
5. 系統自動記錄並備份

### 前台畫面設定

- **主視覺**：標題/副標、Logo 顯示、獎項預覽與進度
- **倒數畫面**：文案、Logo 顯示、獎項名稱/進度、閃名總時長與單次顯示時間
- **中獎畫面**：恭喜文字、Logo、獎項名稱/進度、部門顯示、彩帶、結尾表情
- **設定檔**：可儲存與還原，存檔內容會立即同步前台

### 前台顯示樣式對照

樣式：DisplayScreen.css

待機畫面
.standby-title：主標題
.standby-subtitle：副標題
.prize-preview-name：獎項名稱

倒數畫面
.countdown-number：倒數數字
.countdown-subtitle：倒數副標
.countdown-prize-name：獎項名稱

抽獎畫面
.draw-prize-name：獎項名稱

揭曉畫面
.winner-prize-name：獎項名稱
.winner-name：得獎者姓名
.winner-dept：得獎者部門

### 快捷鍵

- 前台投影視窗：`F11` 切換全螢幕、`Esc` 退出全螢幕

## 重要行為說明

### 未完成活動恢復

啟動時若偵測到未完成的備份資料，系統會詢問是否恢復上次進度。

### 名單插入合併

人員名單支援「插入額外名單」，系統會以「姓名 + 部門」去重，重複者將被略過。

### 快照管理

快照管理目前提供**載入/刪除** `backup_*.xlsx` 檔案。若需建立快照，可將 `backup_state.xlsx` 複製並改名為 `backup_YYYY-MM-DDTHH-MM.xlsx` 後放回 `backup/`。

### 排除已中獎者

- 全域設定可指定「新增獎項時的預設值」
- 每個獎項可獨立切換是否排除已中獎者

## 常見客製化調整（開發者）

以下列出較常被調整的項目與對應檔案位置，方便依需求修改：

| 調整項目 | 檔案位置 | 說明 |
|---|---|---|
| 倒數秒數 | `src/constants/lottery.ts` | 調整 `REVEAL_COUNTDOWN_SECONDS`（`REVEAL_COUNTDOWN_MS` 會自動連動） |
| 倒數畫面文案/結構 | `src/components/display/RevealCountdown.tsx` | 倒數階段的標題、文案與呈現內容 |
| 前台視覺樣式 | `src/components/display/DisplayScreen.css` | 倒數、抽獎、揭曉等前台畫面樣式 |
| 待機畫面標題/副標 | `src/components/display/StandbyScreen.tsx` | 待機畫面標題與提示文字 |
| 中獎揭曉文案/紙花 | `src/components/display/WinnerReveal.tsx` | 祝賀文案、紙花顏色與數量 |
| 抽獎流程與節奏 | `src/components/control/DrawControl.tsx` | 抽獎/確認/重抽的流程節奏與行為 |
| 預設抽獎模式 | `src/stores/lottery-store.ts` | `drawMode`、`customDrawCount`、`globalExcludeWinners` 等預設值 |
| Excel 欄位規則 | `src/utils/excel-handler.ts` | 匯入欄位命名與匯出內容的定義 |
| 備份/快照檔名 | `src/utils/backup-manager.ts` | 備份/快照檔案命名與存檔方式 |
| 前台顯示設定存檔 | `src/utils/display-settings-storage.ts` | 儲存/讀取/清除畫面設定 |
| 全域色系與字體 | `src/index.css` | CSS 變數、字體與共用樣式 |
| 預設音效 | `src/utils/sound-manager.ts` | 內建音效與播放行為設定 |

## 技術架構

- **Electron**：桌面應用框架
- **React + TypeScript**：前端 UI
- **Zustand**：狀態管理
- **xlsx**：Excel 讀寫
- **@dnd-kit**：拖拉排序

## 資料儲存位置

備份檔案與顯示設定（含 Logo/背景/音效）都會存放在 `backup` 資料夾：
- 開發環境：專案根目錄下的 `backup/`
- 打包後（可攜版或有寫入權限時）：`exe` 同層的 `backup/`
- 打包後（`exe` 同層不可寫入時）：回退到啟動時的工作目錄 `backup/`（`process.cwd()`）

> 建議使用 portable 版並放在桌面或 D 槽，使用者可直接備份或替換 `backup` 資料夾內容。

### 備份檔案內容

- `backup_state.xlsx`：完整備份（獎項 / 中獎 / 抽獎池 / 全員名單）
- `backup_participants.xlsx` / `backup_prizes.xlsx` / `backup_winners.xlsx`：拆分備份
- `backup_YYYY-MM-DDTHH-MM.xlsx`：時間戳快照
- `display_settings.json`：前台畫面設定與自訂素材
